...
@interface WPPagedFlowView ()
@property (nonatomic, assign, readwrite) NSInteger currentPageIndex;
@end
@implementation WPPagedFlowView
...
- (void)setPageAtIndex:(NSInteger)pageIndex{
    ...
    if ((NSObject *)cell == [NSNull null]) {
        ...
        if (pageIndex ==1)
        {
            [_scrollView setContentOffset:CGPointMake(_pageSize.width * 1, 0) animated:NO];
        }
    }
}
    ...
    if (_needsReload) {
        ...
        switch (orientation) {
            case PagedFlowViewOrientationHorizontal://横向
                _scrollView.frame = CGRectMake((kMainScreenWidth-_pageSize.width)/2.0, 0, _pageSize.width, _pageSize.height);
